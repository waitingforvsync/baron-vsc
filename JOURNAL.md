...
- Icon: `images/icon.png`, a 16x16 pixel-art owl (amber on navy, ear tufts, folded
  wings) scaled to 128 — an original BBC-Micro-flavoured homage, deliberately NOT the
  BBC's trademarked owl logo. Generated by `tools/gen-icon.js` (pure-node PNG encoder);
  edit the GRID strings there and rerun to change it. Two design lessons from iteration:
  anything centred under the beak reads as a mouth (a ∨ grins, a ∧ frowns), so the chest
  stays plain and the accents went to wing edges instead.
